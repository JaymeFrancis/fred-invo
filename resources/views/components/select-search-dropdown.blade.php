...
<div x-data="select({
    ...
    <span class="inline-block w-full rounded-md shadow-sm">
        <div x-ref="button" @click="toggleListboxVisibility()" :aria-expanded="open" aria-haspopup="listbox"
            class="relative z-0 w-full py-2.5 pl-3 pr-10 text-left transition duration-150 ease-in-out bg-white border border-gray-300 rounded cursor-default sm:text-sm sm:leading-5"
            :class="open && 'border !border-blue-500'">
            ...
            <input x-ref="search" id="search" x-show="open" x-model="search"
                ...
                @keydown.arrow-down.prevent="focusNextOption()" type="search" placeholder="Select an option"
                class="w-full h-full py-0 border-none focus:ring-0 focus:outline-none focus:border-none" />
        ...
        <ul x-ref="listbox" @keydown.enter.stop.prevent="selectOption()"
            ...
            <template x-for="(key, index) in Object.keys(options)" :key="index">
                <li :id="name + 'Option' + focusedOptionIndex" @click="selectOption()"
                    ...
                    :class="{
                        'bg-blue-100 text-blue-700': index === focusedOptionIndex,
                        'text-gray-900': index !==
                            focusedOptionIndex
                    }"
                    ...
                    <span x-show="key === value"
                        :class="{
                            'text-blue-700': index === focusedOptionIndex,
                            'text-blue-700': index !==
                                focusedOptionIndex
                        }"
                        class="absolute inset-y-0 right-0 flex items-center pr-4 text-blue-700">
